...
const BreadcrumbList = React.forwardRef<HTMLOListElement, React.ComponentPropsWithoutRef<"ol">>(({ className, ...props }, ref) => (
  <ol
    ref={ref}
    className={cn(
      "flex flex-wrap items-center gap-1.5 break-words text-sm text-neutralgrey-900 dark:text-neutralgrey-500 sm:gap-2.5",
      className
    )}
    {...props}
  />
...
>(({ asChild, className, ...props }, ref) => {
  ...
  return (
    <Comp ref={ref} className={cn("transition-colors hover:text-neutralgrey-1300 dark:hover:text-neutralgrey-100", className)} {...props} />
  )
})
...
const BreadcrumbPage = React.forwardRef<HTMLSpanElement, React.ComponentPropsWithoutRef<"span">>(({ className, ...props }, ref) => (
  <span
    ...
    aria-disabled="true"
    aria-current="page"
    className={cn("font-normal text-neutralgrey-1300 dark:text-neutralgrey-100", className)}
    {...props}
  />
))
...
const BreadcrumbEllipsis = ({ className, ...props }: React.ComponentProps<"span">) => (
  <span
    role="presentation"
    aria-hidden="true"
    className={cn("flex h-9 w-9 items-center justify-center text-neutralgrey-900", className)}
    {...props}
  >
