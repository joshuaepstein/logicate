import { lighterColour } from "@/lib/color"
...
export default function LightOutputBody({
  ...
  return (
    ...
    >
            ...
            style={{
              stopColor:
                simulated && simulated.state
                  ? item && item.settings.color && item.settings.color !== "#000000"
                    ? lighterColour(item.settings.color, 200)
                    : lighterColour("#1b88e7", 200)
                  : "#fff",
            }}
            offset="0%"
          />
          <stop
            style={{
              stopColor:
                simulated && simulated.state
                  ? item && item.settings.color && item.settings.color !== "#000000"
                    ? lighterColour(item.settings.color, 175)
                    : "#1b88e7"
                  : "#fff",
            }}
            offset="80%"
      ...
      />
      <path
        fill={(item && item.settings.color) || "#000000"}
        stroke="none"
      ...
      />
      <path
        stroke={(item && item.settings.color) || "#000000"}
        strokeWidth="1"
      ...
      <path
        ...
        style={{
          color:
            simulated && simulated.state
              ? item && item.settings.color && item.settings.color !== "#000000"
                ? lighterColour(item.settings.color, 225)
                : "#FFFFFF"
              : (item && item.settings.color) || "#000000",
        }}
        strokeWidth="1"
        strokeLinejoin="round"
        strokeLinecap="round"
