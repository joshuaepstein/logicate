  ...
  return (
    <div className={fileUploaderVariants({ status, size, visibility, className })}>
      ...
      <div className="flex grow flex-row items-center gap-2">
        ...
        {status === 'uploading' && <Progress max={maxProgress || 100} value={progress} className="bg-neutralgrey-400 w-[80px]" />}
        {status === 'uploaded' && <p className="text-neutralgrey-900 text-right text-sm">{humanFileSize(fileSize, true)}</p>}
        ...
        <button onClick={primaryAction}>
          {(status?.includes('uploaded') && <TrashIcon className="text-neutralgrey-900 size-5" />) || (
