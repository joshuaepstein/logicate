        ...
        >
          <Command.Empty className="flex cursor-not-allowed items-center space-x-2 rounded-md bg-neutralgrey-300 px-4 py-2 text-sm text-neutralgrey-800">
            ...
            <div className="flex flex-col space-y-1">
              ...
              <p className="text-xs text-neutralgrey-600">
                Use our AI to find answers on your questions
              </p>
            </div>
          </Command.Empty>
...
const CommandResults = ({
  ...
  return results.map(({ title, description, slug }) => (
    <Command.Item
      ...
      className="group flex cursor-pointer items-center justify-between space-x-2 rounded-md px-4 py-2 hover:bg-neutralgrey-300 active:bg-neutralgrey-400 aria-selected:bg-neutralgrey-300"
    >
      ...
      <ExpandingArrow className="-ml-4 h-4 w-4 text-purple-600 group-aria-selected:visible transition sm:group-hover:visible invisible" />
    </Command.Item>
  ));
};
