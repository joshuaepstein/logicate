      ...
      {split.map((part, i) => {
        ...
        if (active && customEndPathName) {
          return (
            <Breadcrumb
              href={href}
              label={customEndPathName}
              disableSeparator={active}
              key={i}
            />
          );
        }
...
export function BreadcrumbsParent({
  children,
  includeHome,
}: React.ComponentProps<"div"> & { includeHome?: boolean }) {
  return (
...
const BreadcrumbSeparator = () => (
  <ChevronRightIcon className="text-neutralgrey-800 dark:text-neutralgrey-500 size-4" />
);
      ...
      >
        <span
          className={cn("cursor-pointer transition-colors", {
            "text-neutralgrey-800 hover:text-neutralgrey-700 dark:text-neutralgrey-500 dark:hover:text-neutralgrey-600":
              !disabled,
            "text-neutralgrey-600 dark:text-neutralgrey-800 cursor-not-allowed":
              disabled,
            "text-primary-600 hover:text-primary-600 dark:text-primary-400 dark:hover:text-primary-300":
              active_,
          })}
        >
          {label}
