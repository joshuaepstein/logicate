  ...
  return (
    <Container>
      ...
      <div className="flex flex-wrap gap-4">
        {canvas.map((c) => (
          <Link
            className="shadow-hard-2xs flex aspect-square size-40 h-full flex-col items-center justify-start rounded-md transition-all duration-200 hover:scale-105 active:scale-95"
            key={c.id}
            href={`/canvas/${c.id}`}
          >
            <div
