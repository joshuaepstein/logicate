import { AndOperator, BooleanExpression } from '@logicate/types/question/expression/boolean';

export function simplifyBooleanExpression(expression: BooleanExpression): BooleanExpression {
  if (expression.type !== 'compound') return expression;

  const left = simplifyBooleanExpression(expression.left);
  const right = expression.right ? simplifyBooleanExpression(expression.right) : undefined;

  // Apply simplification rules
  if (isConstant(left, true)) return expression.operator === AndOperator ? right || left : left;
  if (isConstant(left, false)) return expression.operator === AndOperator ? left : right || left;
  if (right && isConstant(right, true)) return expression.operator === AndOperator ? left : right;
  if (right && isConstant(right, false)) return expression.operator === AndOperator ? right : left;
