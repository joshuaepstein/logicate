import { FormControlProps, Control as RadixControl } from '@radix-ui/react-form';
...
/**
 * If the render prop is provided, a custom segment input will be rendered. Otherwise a standard input will be rendered.
 */
export const OTPInput = React.forwardRef<HTMLInputElement, OTPInputProps>(function OTPInput(props, ref) {
  ...
  if (hasRenderProp) {
    return <OTPInputSegmented {...rest} ref={ref} render={render} length={length} autoSubmit={autoSubmit} />;
  }

  return <OTPInputStandard {...rest} ref={ref} length={length} autoSubmit={autoSubmit} />;
});

/**
 * Standard `<input />` element that receives the same props as the OTPInput component except for the render prop.
 */
const OTPInputStandard = React.forwardRef<HTMLInputElement, Omit<OTPInputProps, 'render'>>(function OTPInputStandard(props, ref) {
  ...
  return <RadixControl ref={innerRef} {...rest} data-otp-input-standard />;
});
 ...
 * interactions, while the providing render prop is used to visually render the input's contents.
 */
const OTPInputSegmented = React.forwardRef<HTMLInputElement, OTPInputProps>(function OTPInputSegmented(props, ref) {
  const {
  ...
  return (
    <div data-otp-input-wrapper style={wrapperStyle}>
      <style>
        {`
      ...
      <RadixControl
        ...
        onBlur={(event) => {
          setSelectionRange(OUTSIDE);
          rest?.onBlur?.(event);
        }}
        onMouseOver={(event) => {
          if (!isFocused()) setIsHovering(true);
      ...
      />
      <div className={userProvidedClassName} aria-hidden style={segmentWrapperStyle}>
