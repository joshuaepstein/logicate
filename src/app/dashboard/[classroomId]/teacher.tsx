...
export default async function TeacherClassroomDashboard({ user, classroomId }: { user: User; classroomId: string }) {
  const classroom = await getClassroom(classroomId)()
  if (!classroom) notFound()
  const leaderboard = await getLeaderboard(classroomId)

  if (classroom._count.students === 0) {
    return <NoStudents user={user} classroom={classroom} />
  }
    ...
    <main className="max-w-dvw mb-8 flex min-h-[calc(100dvh-15rem)] flex-col">
      ...
      <section className="mt-8 flex h-full w-full grow flex-row gap-8">
        ...
        <div className="flex w-full grow flex-col gap-8">
          <div className="shadow-hard-xs h-full max-h-[50%] grow overflow-y-scroll p-4">
            ...
            <div className="mt-3 flex flex-wrap gap-x-8 gap-y-4">
              {Array.from({ length: 20 }).map((_, index) => (
                <div
                  key={index}
                  className="relative flex h-[82px] w-[130px] flex-col items-start justify-between overflow-hidden rounded-md bg-[#edecfe] px-2.5 py-2 shadow-inner"
                >
                  <p
          ...
          <div className="shadow-hard-xs h-full max-h-[50%] grow p-4">
            <header className="sticky -top-4 z-10 -mt-4 flex w-full flex-col items-start justify-start bg-white pb-2 pt-4">
