...
>(({ className, ...props }, ref) => (
  <AvatarPrimitive.Fallback
    ref={ref}
    className={cn("flex h-full w-full items-center justify-center rounded-full bg-neutralgrey-200 dark:bg-neutralgrey-1100", className)}
    {...props}
  />
))
...
const AvatarIcon = React.forwardRef<React.ElementRef<typeof PersonIcon>, React.ComponentPropsWithoutRef<typeof PersonIcon>>(
  ({ className, ...props }, ref) => <PersonIcon ref={ref} className={cn("h-6 w-6 text-black dark:text-base-white", className)} {...props} />
)
