...
import {
  DownArrowIcon as ArrowDownIcon,
  ArrowUpIcon,
...
>(({ className, ...props }, ref) => (
  <tfoot
    ref={ref}
    className={cn(
      "bg-neutralgrey-200/50 dark:bg-neutralgrey-1100/50 border-t font-medium [&>tr]:last:border-b-0",
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <tr
    ref={ref}
    className={cn(
      "hover:bg-neutralgrey-100 data-[state=selected]:bg-neutralgrey-200 transition-colors",
      className,
    )}
    {...props}
  />
...
const tableCellVariants = cva(
  "transition border-x border-b border-neutralgrey-300 first:border-l-0",
  {
    variants: {
      size: {
        sm: "px-4 h-12",
...
const TableCell = React.forwardRef<HTMLTableCellElement, TableCellProps>(
  ({ className, size, ...props }, ref) => (
    <td
      ref={ref}
      className={tableCellVariants({ size, className })}
      {...props}
    />
  ),
);
...
>(({ className, ...props }, ref) => (
  <caption
    ref={ref}
    className={cn(
      "text-neutralgrey-700 dark:text-neutralgrey-600 mt-4 text-sm",
      className,
    )}
    {...props}
  />
...
const TableText = React.forwardRef<
  ...
  React.HTMLAttributes<HTMLParagraphElement> &
    VariantProps<typeof tableTextVariants>
>(({ className, variant, ...props }, ref) => (
  <p
    ref={ref}
    className={cn(tableTextVariants({ variant }), className)}
    {...props}
  />
));
